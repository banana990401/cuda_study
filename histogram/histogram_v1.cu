...
// 1.174400 ms
// tips:
// L68和L75为遇见的两个bug，说明了在遍历的时候需要精确传入数据量，多了或少了都可能会出现垃圾值或者cuda干脆对这种情况不处理
...
__global__ void histgram(int* hist_data, int* bin_data, int N)
{
    ...
    for(int i = gtid; i < N; i += gridDim.x * blockSize)
    {
        ...
        atomicAdd(&cache[val],
                  1); // 原子加法，强行使得并行的CUDA线程串行执行加法，但是并不能保证顺序
    }
...
int main()
{
    ...
    // bug2:
    // 同bug1，L67传进去的256表示两个buffer的数据量，这个必须得精确，之前传的N，尽管只打印第1个值，但依然导致L27打印出来的值为垃圾值
