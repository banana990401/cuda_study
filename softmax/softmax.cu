...
template <template <typename> class ReductionOp, typename T, int warp_size = WarpSize>
...
template <typename T>
__inline__ __device__ T Divide(T a, T b);

template <>
__inline__ __device__ float Divide(float a, float b)
{
...
// 1, 1024/32,32, 1
template <int pack_size, int cols_per_thread, int warp_size, int rows_per_thread>
__global__ void WarpSoftmax(const float* src, float* dst, const int rows, const int cols)
{
    constexpr int num_packs = cols_per_thread / pack_size; // 一行有多少个向量
    assert(cols <= cols_per_thread * warp_size);
    float buf[rows_per_thread][cols_per_thread];

    const int global_warp_id  = blockIdx.y * blockDim.y + threadIdx.y;
    const int num_global_warp = gridDim.y * blockDim.y; // 1000
    ...
    for(int row = global_warp_id * rows_per_thread; row < rows; row += step)
    {
        ...
        for(int row_id = 0; row_id < rows_per_thread; row_id++)
        {
            ...
            float* row_buf     = buf[row_id]; //row_id行起始位置
            ...
            for(int pack_id = 0; pack_id < num_packs; pack_id++)
            {
                ...
                // 当前向量所在的起始列号
                const int col = (pack_id * warp_size + lane_id) * pack_size;
                if(col < cols)
                {
                    ...
                    {
                        thread_max[row_id] = max(row_buf[pack_offset + i], thread_max[row_id]); // 当前向量中的最大值
                    }
                }
                else
                {
        ...
        for(int row_id = 0; row_id < rows_per_thread; row_id++)
        {
            warp_max[row_id] = WarpReduce<MaxOp, float, warp_size>(thread_max[row_id]);
        }
        ...
        for(int row_id = 0; row_id < rows_per_thread; ++row_id)
        {
            warp_sum[row_id] = WarpReduce<SumOp, float, warp_size>(thread_sum[row_id]);
        }
        ...
        for(int row_id = 0; row_id < rows_per_thread; ++row_id)
        {
            ...
            for(int i = 0; i < cols_per_thread; ++i)
            {
                row_buf[i] = Divide(row_buf[i], warp_sum[row_id]);
            }
            // 哪里来回哪里去，把最终结果写回显存
            for(int i = 0; i < num_packs; ++i)
            {
                const int col = (i * warp_size + lane_id) * pack_size;
                if(col < cols)
                {
