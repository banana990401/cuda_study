...
int geluCpu(float x)
{
    return 0.5 * x * (1.0 + tanh(sqrt(2.0 / M_PI) * (x + 0.04471499845385551 * x * x * x)));
}
...
struct GeluFunctor<half>
{
    ...
    {
        // improvement by half intrinsic instead of static_cast half to fp32.
        const float tanh_in =
    ...
    // improvement by half2 intrinsic.
    __device__ void apply2(half* y, const half* x) const
    {
        ...
        float2 tanh_out;
        tanh_out.x =
            TanhApprox(tanh_in.x); // tanh之所以转为fp32类型计算，是因为NV GPU貌似不支持tanh的half
                                   // intrinsic，理想状况下，当然是希望所有计算都是half2一把梭
...
__global__ void FP16GeluCUDAKernel(const __half* x, __half* y, int n)
{
    ...
    for(; offset < n; offset += stride)
    {
        ...
        {
            // to get performance improvement by half2 intrinsic do vector computation.
